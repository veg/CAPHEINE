...
| `--foreground_list`   | Path to a text file with a newline-separated list of foreground taxa. Taxa in this list will be sanitized to match the hyphy-sanitized taxon name (all non-alphanumeric, non-underscore characters are replaced with '\_'). Only one of `foreground_list` or `foreground_regexp` should be provided per row.                                                                                                                                                                                                                                                                                                        | No       |
| `--foreground_regexp` | Regular expression to match foreground taxa. Only one of `foreground_list` or `foreground_regexp` should be provided per row. Be aware that the regular expression is matched to the hyphy-sanitized taxon name (all non-alphanumeric, non-underscore characters are replaced with '\_'), not the raw taxon name.                                                                                                                                                                                                                                                                                                   | No       |
